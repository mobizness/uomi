...
const UIEdgeInsets textInsetsMine = {15, 20, 21, 27};
...
//    UIFont *font = [UIFont systemFontOfSize:[UIFont systemFontSize]];
    UIFont *font = [UIFont fontWithName:@"Averta-Regular" size:14];
...
+ (id)dataWithImageURL:(NSString *)url date:(NSDate *)date type:(NSBubbleType)type
{
#if !__has_feature(objc_arc)
    return [[[NSBubbleData alloc] initWithImageURL:url date:date type:type] autorelease];
#else
    return [[NSBubbleData alloc] initWithImageURL:url date:date type:type];
#endif
}

- (id)initWithImageURL:(NSString *)url date:(NSDate *)date type:(NSBubbleType)type
{
    CGFloat height = 220;
    CGFloat width = 220;
    
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, width, height)];
    [commonUtils setImageViewAFNetworking:imageView withImageUrl:url withPlaceholderImage:[UIImage imageNamed:@"blank"]];
    imageView.layer.cornerRadius = 5.0;
    imageView.layer.masksToBounds = YES;
